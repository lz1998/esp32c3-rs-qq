...
pub fn t1(uin: u32, ip: &[u8]) -> Bytes {
    ...
    buf.write_bytes_short(&{
        ...
        let rand = crate::get_random_provider();
        w.put_u16(1);
        // w.put_u32(rand::random());
        w.put_u32(rand.next_u32());
        w.put_u32(uin);
        // w.put_u32(Utc::now().timestamp_millis() as u32);
        w.put_u32(crate::get_timer_provider().now_timestamp_millis() as u32);
        w.put_slice(ip);
        w.put_u16(0);
        w.freeze()
    });
...
pub fn t106(
    ...
    let body = &{
        let mut w = BytesMut::new();
        let rand_provider = crate::get_random_provider();
        w.put_u16(4);
        // w.put_u32(rand::random::<u32>());
        w.put_u32(rand_provider.next_u32());
        w.put_u32(sso_ver);
        w.put_u32(16); // appId
        w.put_u32(0); // app client version
        if uin == 0 {
...
pub fn t400(
    ...
    buf.write_bytes_short(&{
        ...
        w.encrypt_and_write(g, &{
            ...
            ww.put_u32(crate::get_timer_provider().now_timestamp_millis() as u32);
...
mod tests {
    ...
    fn init() {
        ...
        crate::init_test_timer_provider();
    }
