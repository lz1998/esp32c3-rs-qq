...
mod test {
    ...
    fn test_compress() {
        init();
        let app = LightApp{content:"Hello world".to_string()};
        let ele = Vec::<msg::elem::Elem>::from(app);
        match &ele[0] {
            msg::elem::Elem::LightApp(app) => {
                assert_eq!(app.data,Some(vec![1, 120, 156, 243, 72, 205, 201, 201, 87, 40, 207, 47, 202, 73, 1, 0, 24, 171, 4, 61]));
    ...
    fn test_decompress() {
        init();
        let data = vec![1u8, 120, 156, 243, 72, 205, 201, 201, 87, 40, 207, 47, 202, 73, 1, 0, 24, 171, 4, 61];
        let elem = msg::LightApp{data:Some(data),msg_resid:None};
