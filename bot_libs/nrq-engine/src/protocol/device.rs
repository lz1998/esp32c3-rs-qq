...
impl Device {
    pub fn random() -> Self {
        let rng = crate::get_random_provider();
        Self {
            ...
            imsi_md5: md5::compute(crate::get_random_provider().gen::<[u8; 16]>()).to_vec(),
            android_id: encode_hex(&crate::get_random_provider().gen::<[u8; 8]>()),
            apn: "wifi".into(),
...
pub fn random_uuid() -> String {
    let r = md5::compute(&crate::get_random_provider().gen::<[u8; 16]>()).to_vec();
    format!(
        "{}-{}-{}-{}-{}",
        encode_hex(&r[0..4]),
        encode_hex(&r[4..6]),
