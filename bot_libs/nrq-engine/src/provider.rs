...
use crate::{CoreCryptoRng, crypto};
static mut RANDOM_PROVIDER:core::mem::MaybeUninit<Box<dyn CoreCryptoRng>> = core::mem::MaybeUninit::uninit();
...
pub fn init_test_random_provider() {
    init_random_provider(Box::new(crypto::ForceCryptoRng::new(rand::rngs::mock::StepRng::new(1,3))));
}
pub fn get_random_provider() -> &'static mut Box<dyn CoreCryptoRng> {
    unsafe {
        &mut *RANDOM_PROVIDER.as_mut_ptr()
    }
}
...
static mut TIMER_PROVIDER:core::mem::MaybeUninit<Box<dyn TimerProvider>> = core::mem::MaybeUninit::uninit();
