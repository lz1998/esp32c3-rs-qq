...
pub trait TSender<T:Clone + Send>:Clone + Send + Sync {
    ...
    type SenderFuture<'a>:Future<Output = Result<usize,Self::Error>> + Send + 'a where Self:'a;
    fn send(&self,value:T) -> Self::SenderFuture<'_>;
    fn is_closed(&self) -> bool;
    fn close_channel(&self) -> ();
    fn subscribe(&self) -> Self::Receiver;
}
pub trait TReceiver<T>:Send + Sync where T:Send + Clone{
    type Error:Debug + Send = ();
    type RecvFuture<'a>:Future<Output = Result<T,Self::Error>> + futures::future::FusedFuture + Send + 'a where Self:'a;
    // fn close(&self) -> ();
...
pub trait TRwLockWriteGuard<'a,T:?Sized>: core::ops::Deref<Target = T> + core::ops::DerefMut {
    
}
pub trait TRwLock<T:?Sized>:Send + Sync {
    type ReadGuard<'a>:TRwLockReadGuard<'a,T> + Send where Self:'a;
    type WriteGuard<'a>:TRwLockWriteGuard<'a,T> + Send where Self:'a;
    type ReadFuture<'a>:Future<Output = Self::ReadGuard<'a>> + Send where Self:'a;
    type WriteFuture<'a>:Future<Output = Self::WriteGuard<'a>> + Send where Self:'a;
    fn new(value:T) -> Self;
    fn read(&self) -> Self::ReadFuture<'_>;
    fn write(&self) -> Self::WriteFuture<'_>;
}
pub trait RwLockProvider:Sync {
    type RwLock<T>:TRwLock<T> where T:Send + Sync;
}
...
pub trait TaskProvider:Sync {
    ...
    type TimeoutFuture<T:Future+Send>:Future<Output = Result<T::Output,Self::TimeoutError>> + Send;
    ...
    fn timeout<T:Future+Send>(duration:core::time::Duration,future:T) -> Self::TimeoutFuture<T>;
...
pub trait AsyncRead {
    type IOError:Debug = ();
    fn poll_read(self:Pin<&mut Self>,cx:&mut Context<'_>,buf:&mut ReadBuf<'_>) -> Poll<Result<(),Self::IOError>>;
}
pub trait AsyncWrite {
    type IOError:Debug + From<RQError> = ();
    fn poll_write(self:Pin<&mut Self>,cx:&mut Context<'_>,buf:&[u8]) -> Poll<Result<usize,Self::IOError>>;
    fn poll_flush(self:Pin<&mut Self>,cx:&mut Context<'_>) -> Poll<Result<(),Self::IOError>>;
    fn poll_shutdown(self:Pin<&mut Self>,cx:&mut Context<'_>) -> Poll<Result<(),Self::IOError>>;
}
...
macro_rules! deref_async_write {
    () => {
        ...
        fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(),Self::IOError>> {
            Pin::new(&mut **self).poll_flush(cx)
        }

        fn poll_shutdown(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(),Self::IOError>> {
...
impl AsyncWrite for alloc::vec::Vec<u8> {
    type IOError = ();
    fn poll_write(self:Pin<&mut Self>,_cx:&mut Context<'_>,buf:&[u8]) -> Poll<Result<usize,Self::IOError>> {
    ...
    fn poll_shutdown(self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Result<(),Self::IOError>> {
        Poll::Ready(Ok(()))
    }
}
...
pub trait TMutex<T>:Send+Sync {
    type Guard<'a>:TMutexGuard<'a,T> + Send where Self:'a;
    type Future<'a>:Future<Output = Self::Guard<'a>> + Send where Self:'a;
    fn new(value:T) -> Self;
