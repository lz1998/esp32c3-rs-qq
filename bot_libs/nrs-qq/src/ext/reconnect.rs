...

{
    ...
    fn connect(&self,client:&Arc<Client<CP, OSCP, RP, MP, TP, TCP>>) -> Self::ConnectFuture {
        let c = client.clone();
        async move {
            TCP::connect(c.get_address()).await
        }
    }
}

/// 自动重连，在掉线后使用，会阻塞到重连结束
pub async fn auto_reconnect<CP, OSCP, RP, MP, TP, TCP,C: Connector<CP, OSCP, RP, MP, TP, TCP> + Sync>(
...

{
    ...
    fn fast_login(&self,client:Arc<Client<CP, OSCP, RP, MP, TP, TCP>>) -> Self::LoginFuture {
        let s = self.clone();
        async move {
            client.token_login(s.0.clone()).await
        }
    }
}
