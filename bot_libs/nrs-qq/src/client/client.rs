     ...
     
{
    ...
    where
        H:crate::handler::Handler<CP,OSCP,RP,MP,TP,TCP,Future = core::pin::Pin<Box<dyn futures::Future<Output = ()> + Send>>> + 'static + Sync + Send
    {
        let (out_pkt_sender,_) = CP::channel(1024);
    ...
    
{
    ...
    pub async fn send_and_wait(&self, pkt: Packet) -> RQResult<Packet> {
        ...
        let (sender, receiver) = OSCP::channel();
        {
