...
use nrq_engine::command::message_svc::MessageSyncResponse;
use nrq_engine::command::oidb_svc::music::{MusicShare, MusicType, SendMusicTarget};
use nrq_engine::common::group_code2uin;
use nrq_engine::msg::elem::Anonymous;
use nrq_engine::msg::MessageChain;
use nrq_engine::pb;
use nrq_engine::{get_random_provider, get_timer_provider};
use rand::RngCore;
    ...
    
{
    ...
    pub async fn send_group_message(
        ...
        let timer_provider = get_timer_provider();
        let time = timer_provider.now_timestamp();
        // let ran = (rand::random::<u32>() >> 1) as i32;
        let ran = (get_random_provider().next_u32() >> 1) as i32;
        let (tx, rx) = OSCP::channel();
        {
            self.receipt_waiters.lock().await.insert(ran, tx);
            // self.receipt_waiters.lock().await;
        }
        ...
        match TP::timeout(core::time::Duration::from_secs(5), rx).await {
            Ok(Ok(seq)) => {
    ...
    // /// 通过群号获取群
    pub async fn find_group(&self, code: i64, auto_reload: bool) -> Option<Arc<Group<RP>>> {
    ...
    pub async fn get_group_infos(&self, group_codes: Vec<i64>) -> RQResult<Vec<GroupInfo>> {
        ...
        pub async fn reload_group(&self, group_code: i64) -> RQResult<()> {
            ...
            groups.insert(
                ...
                // Arc::new(Group {
                //     info: group_info,
                //     members: RP::RwLock::new(members),
                // }),
                Arc::new(Group::<RP>::new(group_info, RP::RwLock::new(members)))
            );
            Ok(())
        }
        ...
        pub async fn reload_groups(self: &Arc<Self>) -> RQResult<()> {
            ...
            let group_list: Vec<(i64, Arc<Group<RP>>)> = stream::iter(groups)
                .map(|g| async move {
                    ...
                    (
                        ...
                        // Arc::new(Group {
                        //     info: g,
                        //     members:TP::RwLock::new(mem_list),
                        // }),
                        Arc::new(Group::<RP>::new(g, RP::RwLock::new(mem_list)))
                    )
                })
                .buffered(10)
                .collect()
        ...
        pub async fn send_private_message(
            ...
            let time = nrq_engine::get_timer_provider().now_timestamp();
            let seq = self.engine.read().await.next_friend_seq();
            // let ran = (rand::random::<u32>() >> 1) as i32;
            let ran = (nrq_engine::get_random_provider().next_u32() >> 1) as i32;
