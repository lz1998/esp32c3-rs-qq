use std::{sync::{Arc,atomic::{AtomicBool, Ordering}}, task::{RawWaker,RawWakerVTable, Waker}, mem};
#[derive(Debug,Clone)]
pub struct RawTask(Arc<AtomicBool>);


impl RawTask {
    const RAW_WAKER_VTABLE:RawWakerVTable = RawWakerVTable::new(Self::clone_waker, Self::wake, Self::wake_by_ref, Self::drop_waker);
    pub fn new() -> Self {
    ...
    pub fn to_waker(&self) -> Waker {
        unsafe {
            Waker::from_raw(self.to_raw_waker())
        }
    }
