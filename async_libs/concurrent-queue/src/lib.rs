...
impl<T> ConcurrentQueue<T> {
    ...
    pub fn bounded(cap: usize) -> ConcurrentQueue<T> {
        log::info!("queue bounded call cap {} s {:?}",cap,std::thread::current().name());
        if cap == 1 {
    ...
    pub fn type_str(&self) -> &'static str {
        match &self.0 {
            ...
            Inner::Bounded(_) => "bounded",
            Inner::Unbounded(_) => "unbounded"
        }
    }

    pub fn reset_to_unbounded(&self) {
        match &self.0 {
            Inner::Single(_) => {

                unsafe {
                    // 我的esp32中这里的数据有概率会变成0也就是Inner::Single，大多发生在时间定时之后，暂时不知道为什么会这样。
                    // 目前的解决方案是直接写入1
                    let point =self as *const _ as *mut u8;
                    point.write(1);
                }
            }
            _ => {
