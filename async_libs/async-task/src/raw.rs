    ...
    S: Fn(Runnable),
{
    ...
    unsafe fn wake(ptr: *const ()) {
        ...
        if mem::size_of::<S>() > 0 {
            ...
            Self::drop_waker(ptr);
            log::debug!("wake() done1");
            return;
        }

        // log::debug!("to_read_ptr");
        ...
        // log::debug!("wake() done2");
    }

    /// Wakes a waker by reference.
    unsafe fn wake_by_ref(ptr: *const ()) {
        // log::debug!("wake_by_ref()");
        let raw = Self::from_ptr(ptr);
    ...
    unsafe fn run(ptr: *const ()) -> bool {
        ...
        log::debug!("run() raw from waker");
        let mut state = (*raw.header).state.load(Ordering::Acquire);
        log::debug!("run() raw load state");
