...
//use no_atomic::{AtomicBool, AtomicUsize};
...
impl Poller {
    ...
    pub fn delete(&self, fd: RawFd) -> io::Result<()> {
        log::info!("delete: notify_read={}, fd={}", self.notify_read, fd);
    ...
    pub fn notify(&self) -> io::Result<()> {
        log::info!("notify: notify_read={}", self.notify_read);
...
impl Drop for Poller {
    fn drop(&mut self) {
        log::info!("drop: notify_read={}", self.notify_read);
