...
pub fn test_condvar() {
    ...
    log::info!("wait timeout run");
    //thread::sleep(Duration::from_secs(1));
    log::info!("wait timeout ok");
    // match c.wait_timeout(guard, std::time::Duration::from_secs(2)) {
    //     Ok((g,r)) => {
    //         log::info!("{:?}", g);
    //         log::info!("{:?}", r);
    //     },
...
impl Inner {
    fn park(&self, timeout: Option<Duration>) -> bool {
        log::info!("Inner park() thread name {}",thread::current().name().unwrap());
